...
function Product() {
  ...
  const fetchData = async () => {
    try {
      ...
      const authorsList = await axios.get(
        config.apiPath + "/product/authors/",
        config.headers()
      );
      ...
      const categoriesList = await axios.get(
        config.apiPath + "/product/categories/",
        config.headers()
      );
  ...
  const handleSave = async () => {
    try {
      ...
      if (!product.quantity || product.quantity < 0)
        errorListInFront.push("quantity");
      console.log("errorList: " + errorListInFront);
      if (errorListInFront.length > 0) throw new Error("410");

      if (isNewAuthor) {
        const newAuthor = await axios.post(
          config.apiPath + "/product/createAuthor/",
          { name: selectedAuthor },
          config.headers()
        );
      ...
      if (isNewCategory) {
        const numbers = selectedCategory.filter(
          (item) => typeof item === "number"
        );
        const strings = selectedCategory.filter(
          (item) => typeof item === "string"
        );
        const newCategory = await axios.post(
          config.apiPath + "/product/createCategory/",
          { name: strings },
          config.headers()
        );
      ...
      if (e.message === "410" || e.response.status === 410) {
        const errorList =
          e.message === "410" ? errorListInFront : e.response.data["errorList"];
  ...
  const onSelectedCategoryChange = (category) => {
    ...
    Object.values(category).some((value) => typeof value === "string")
      ...
      >
        ...
        <div className="mt-1">
          ...
          <TextArea
            rows={3}
            allowClear
            placeholder="type some details of the product"
            value={product.desc}
          ...
          {isEdit && !isRemoveIMG ? (
            ...
            >
              <Image
                ...
                height={200}
                width="full"
                src={config.apiPath + "/uploads/product_img/" + product.img}
                fallback="default_img.webp"
                preview={false}
              />
