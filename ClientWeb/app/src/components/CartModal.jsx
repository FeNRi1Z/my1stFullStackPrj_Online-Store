					...
					<Tooltip
						...
						<div className="flex items-center gap-2 sm:gap-4">
							...
							<div className="min-w-0 flex-1">
								<h3 className="font-medium text-sm sm:text-base line-clamp-1 sm:line-clamp-2" style={{ color: themeStyles.text }}>
									{text}
								</h3>
								<p className="text-xs sm:text-sm text-gray-600 dark:text-gray-400 line-clamp-1">{record.author}</p>
							</div>
						</div>
					</Tooltip>
				),
				...
				render: (_, record) => (
					<div className="flex items-center justify-center gap-1 sm:gap-2">
						...
						<Tooltip title={`${record.quantity} of ${record.maxQuantity || 99} available`}>
							<span className="w-6 sm:w-8 text-center text-sm sm:text-base">{record.quantity}</span>
						</Tooltip>
						<Button
							size="small"
							icon={<PlusOutlined />}
			...
			baseColumns.push(
				{
					...
					width: "15%",
					sorter: (a, b) => a.quantity - b.quantity,
					render: (price) => <div className="text-center">${price.toFixed(2)}</div>,
				},
				{
					title: "Total",
					dataIndex: "total",
					...
					width: "15%",
					sorter: (a, b) => a.quantity - b.quantity,
					render: (_, record) => <div className="text-center">${(record.price * record.quantity).toFixed(2)}</div>,
				}
			);
		}
            ...
            ${isAnimating ? "bg-opacity-50" : "bg-opacity-0"}`}
              ...
              ${isAnimating ? "translate-y-0 opacity-100 scale-100" : "translate-y-4 opacity-0 scale-95"}`}
                ...
                ${isAnimating ? "opacity-100" : "opacity-0"}`}
							...
							<Button type="text" icon={<CloseOutlined />} onClick={handleClose} className="absolute right-2 sm:right-4 top-2 sm:top-4 hover:text-primary-100 focus:text-primary-100" />
                ...
                ${isAnimating ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}
							...
							{loading ? (
								...
								<div className="text-center py-8 sm:py-12 px-4">
									<ShoppingBag className="mx-auto mb-4" size={48} strokeWidth={1.5} style={{ color: primaryColor }} />
