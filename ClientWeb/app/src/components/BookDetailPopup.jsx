...
const BookDetailPopup = ({ book, onClose }) => {
	...
	return (
		...
		<div className={`fixed inset-0 z-50 flex items-center justify-center p-4 overflow-y-auto transition-opacity duration-300 ease-in-out`}>
			{/* Backdrop */}
			<div
			...
			<div
				className="relative bg-white dark:bg-background-secondary-dark rounded-lg w-full max-w-3xl 
					...
					<div className="flex flex-col sm:flex-row gap-6">
						...
						<div className="flex flex-col sm:flex-shrink-0 items-center sm:items-start">
							<div className="relative w-[180px] h-[250px] sm:w-[200px] sm:h-[280px] shadow-lg rounded-lg overflow-hidden">
								<Image src={book.cover} alt={`Cover of ${book.title}`} className="w-full h-full object-cover" />
							</div>
							<div className="mt-4 text-center sm:text-left space-y-1 w-full">
								<h2 className="text-xl font-semibold text-gray-900 dark:text-white line-clamp-2">{book.title}</h2>
						...
						<div className="flex-1 flex flex-col min-w-0">
							...
							<div className="flex-1">
								...
								<p className="text-gray-600 dark:text-gray-400 text-sm sm:text-base">{book.desc}</p>
							</div>
							{/* Categories tag */}
							<div className="flex-1">
							...
							<div className="mt-6 pt-4">
								<div className="flex flex-col sm:flex-row gap-4 sm:items-center sm:justify-between">
									...
									<div className="flex items-center justify-between sm:justify-start gap-4">
										<div className="flex items-center space-x-2 bg-gray-50 dark:bg-gray-800 rounded-lg p-1">
											<button
												...
												className={`p-1.5 rounded-md transition-colors duration-200 ${
													quantity <= 1 ? "text-gray-400 dark:text-gray-500 cursor-not-allowed" : "text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700"
												}`}
												disabled={quantity <= 1}
												aria-label="Decrease quantity">
												<Minus className="w-4 h-4" />
											</button>
											<span className="w-8 text-center text-gray-900 dark:text-white font-medium">{quantity}</span>
											<button
										...
										<div className="text-lg font-semibold text-gray-900 dark:text-white">${(book.price * quantity).toFixed(2)}</div>
									</div>
